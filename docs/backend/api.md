...
Search public articles by title or content, ordered by publish_at descending.

按标题或正文搜索公开文章，结果按 publish_at 降序排列。

**Authentication / 鉴权:** None required / 无需鉴权

**Query Parameters / 查询参数:**
- `q` (string, optional) - Search keyword (empty searches all public articles) / 搜索关键词（为空时搜索所有公开文章）
- `page_size` (integer, optional) - Maximum returned items (default: 20) / 最大返回项目数（默认：20）

**Response / 响应:** `200 OK` (Same format as `GET /api/public/articles` / 格式同 `GET /api/public/articles`)

**Notes / 注意事项:**
- Only publicly listed articles (`is_public = true`) are included / 仅包含公开列出的文章（`is_public = true`）
- `accessible = true` only when `required_tier = 0` / 仅当 `required_tier = 0` 时 `accessible = true`

---

### GET /api/public/articles/search/page
Get total pages and item count for public article search results.
...
### GET /api/public/articles/search/page/:page
List paginated public article search results.
